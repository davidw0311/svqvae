...
if __name__ == '__main__':
    ...
    directory_name = f"runs/train-svqvae-withmask-{dataset}-" + current_time
    ...
    for e in range(1, num_epochs+1):
        ...
        for step, (image_batch, mask_batch, labels_batch) in enumerate(train_loader):
            ...
            # loss between the latent of image and latent of overlayed image
            qt, qb, qj, diff, idt, idb = model.encode(image_batch, 0, num_vaes-1)
            with torch.no_grad():
                _, _, qj_overlay, _, _, _ = model.encode(overlay_batch, 0, num_vaes-1)
            
            feature_loss = mse_loss(qj, qj_overlay)
